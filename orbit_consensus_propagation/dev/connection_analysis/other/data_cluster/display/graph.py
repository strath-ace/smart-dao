...
ax.plot(li_sat_names, y_array_1_out, c="green", label="Total number of connected timesteps")
ax.set_xlabel("Satellite name")
ax.set_ylabel("Counted connections of time period", c="green")
ax.set_ylim([0, 1.1*np.amax(y_array_1_out)])


ax2=ax.twinx()
ax2.plot(li_sat_names, [li_unique[i] for i in ind], c="blue")
ax2.set_ylabel("Num of unique combinations", c="blue")
ax2.set_ylim([0, len(li_sat_names)])
ax2.yaxis.set_major_locator(MaxNLocator(integer=True))

title_string = "Absolute number of connections (Computed over "+str(total_timer)+" hours with "+str(len(li_sat_names))+" cluster mission satellites)"
...
ax.plot(li_sat_names, y_array_2_out, label="Total timesteps connected in time period", c="green")
ax.set_xlabel("Satellite name")
...
ax2=ax.twinx()
ax2.plot(li_sat_names, [li_unique[i] for i in ind],  label="Unique connections in time period", c="blue")
ax2.set_ylabel("Num of unique combinations", c="blue")
ax2.set_ylim([0, len(li_sat_names)])
ax2.yaxis.set_major_locator(MaxNLocator(integer=True))

title_string = "How connected to the number of unqiue sats (Computed over "+str(total_timer)+" hours with "+str(len(li_sat_names))+" cluster mission satellites)"
plt.title(title_string)

plt.savefig(save_location+"/num_of_connections_percentage_of_max.png")
plt.savefig(save_location+"/../../data/connections_cluster.png")
# plt.show()
