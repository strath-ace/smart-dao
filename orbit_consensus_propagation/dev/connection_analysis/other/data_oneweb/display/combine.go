...
func combine(){
	...
	for _, sat_file := range file_sats {
		...
		sat_names = append(sat_names, sat_file.Name()[:len(sat_file.Name())-4])
		total_conns = append(total_conns, strconv.Itoa(total_count))
		unique_conns = append(unique_conns, strconv.Itoa(unique_count))
	}
	save_data(sat_names, total_conns, unique_conns, graph_data_file)
}
...
func save_data(out_data1, out_data2, out_data3 []string, file_name string) {
