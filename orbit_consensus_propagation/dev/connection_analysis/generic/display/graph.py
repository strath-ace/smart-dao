...
li_nums = np.asanyarray(li_all, dtype=float)

# Sort by li_nums[:,0] first then by li_num[:,1] (aka sort by unique then total)
ind = np.lexsort((li_nums[:,0], li_nums[:,1]), axis=0)
...
ax.set_ylabel("Counted connections of time period")
ax.set_ylim([0, 1.1*np.amax(y_array_1_out)])

ax.plot([li_unique[i] for i in ind], label="Unique connections in time period", c="blue")

title_string = "Absolute number of connections (Computed over "+str(total_timer)+" hours with all LEO satellites)"
...
ax.set_ylim([0, 1.1*np.amax(y_array_2_out)])
...
ax2.set_ylim([0, 1.1*np.amax(li_unique)])

title_string = "How connected to the number of unqiue sats (Computed over "+str(total_timer)+" hours with all LEO satellites)"
plt.title(title_string)
