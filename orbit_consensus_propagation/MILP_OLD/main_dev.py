...
for j in range(n):
    ...
    for i in range(n):
        ...
        # T = T-1
        for t in range(np.shape(T)[2]):
            n_x = m.addVar(vtype=GRB.INTEGER, lb=-GRB.INFINITY)
            m.addConstr(n_x == 10*(T[i,j,t] - H1[i])+5)    # +5 gets away from zero
            exped = m.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=1)
            m.addGenConstrLogistic(n_x, exped)
            t_temp = m.addVar(vtype=GRB.CONTINUOUS)
            m.addConstr(t_temp == (T[i,j,t]*(exped)+(3000*(1-exped))))
            t0.append(t_temp)

        m.addGenConstrMin(t_send, t0)

        m.addConstr(t_required == (t_send-1) * X[i,j,1])
...
for j in range(n):
    ...
    for i in range(n):
        ...
        t0 = []
        for t in range(np.shape(T)[2]):
            n_x = m.addVar(vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY,ub=GRB.INFINITY)
            m.addConstr(n_x == 10*(T[i,j,t] - H2[i])+5)    # +5 gets away from zero
            exped = m.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=1)
            m.addGenConstrLogistic(n_x, exped)
            t_temp = m.addVar(vtype=GRB.CONTINUOUS)
            m.addConstr(t_temp == (T[i,j,t]*(exped)+(3000*(1-exped))))
        ...
        m.addConstr(t_required == (t_send-1) * X[i,j,2])
...
for j in range(n):
    ...
    for i in range(n):
        ...
        for t in range(np.shape(T)[2]):
            n_x = m.addVar(vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY,ub=GRB.INFINITY)
            m.addConstr(n_x == 10*(T[i,j,t] - H3[i])+5)    # +5 gets away from zero
            exped = m.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=1)
            m.addGenConstrLogistic(n_x, exped)
            t_temp = m.addVar(vtype=GRB.CONTINUOUS)
            m.addConstr(t_temp == (T[i,j,t]*(exped)+(3000*(1-exped))))
...
try:
    print("H1")
    print(*[H1[i].getAttr("x") for i in range(n)])
except:
    pass
try:
    print("H2")
    print(*[(H2[i].getAttr("x")) for i in range(n)])
except:
    pass
try:
    print("H3")
    print(*[(H3[i].getAttr("x")) for i in range(n)])
except:
    pass
try:
    print("H4")
    print(*[(H4[i].getAttr("x")) for i in range(n)])
except:
    pass
# # print("Final Times")
