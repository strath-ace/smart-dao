...
for i in [0,1,2,3,4,5]:
    ...
    bigger = len(s[:,i])
    wid = [1, math.sqrt(len(div100[:,i])/bigger), math.sqrt(len(div50[:,i])/bigger), math.sqrt(len(div25[:,i])/bigger), math.sqrt(len(div1[:,i])/bigger)]
    ...
    violin = plt.violinplot([s[:,i], div100[:,i], div50[:,i], div25[:,i], div1[:,i]], [0,1,2,3,4], points=100, widths=wid, showmeans=True, showextrema=True, showmedians=False)
    # plt.xlabel(headings_labels[x_val+1])#
    plt.xticks([0,1,2,3,4], ["Major Subset (n)", "Lower 50% of Satellites", "Upper 50% of Satellites", "Top 20% of Satellites", "Most common Satellite"], rotation=-10)
    ## Explanation
...
plt.xticks(np.arange(len(all_uniq_c)), np.array(all_uniq,dtype=int))
...
plt.xticks(np.arange(len(unique_sats)), unique_sats)
plt.yticks(np.arange(len(unique_sats)), unique_sats)
...
#     plt.tight_layout()
#     plt.imshow(np.swapaxes(big_T[i,:,:], 0,1), aspect='auto', interpolation='none')
#     # for j in range(len(unique_sats)):
#         # if i == unique_sats[j]:
#     sat_name = sat_attributes_new[i,0]
#     plt.title("Satellite interactions for satellite "+str(unique_sats[i])+" - "+sat_name)
#     plt.ylabel("Timesteps")
#     plt.xlabel("Interactions with other satellites")
#     plt.xticks(np.arange(len(unique_sats)),unique_sats)
#     plt.gca().invert_yaxis()
#     plt.savefig(save_location+"/satellite_interactions_"+str(unique_sats[i])+".png")
#     plt.clf()


fig, axs = plt.subplots(5,6, figsize=(24,20),sharex=True, sharey=True)
print(np.shape(big_T))
j = 0
k = 0
for i in np.arange(30):
    ...
    axs[k,j].set_title("Sat "+str(unique_sats[i])+" - "+sat_name)
