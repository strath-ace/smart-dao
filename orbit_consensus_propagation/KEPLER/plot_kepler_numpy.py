...
# results_file = "all_combs_100.csv"

file_names = "data_tester"
name_names = ["All"]
...
results_location = os.path.join(save_location, RESULTS_DIR)
...
while True:
    try:
        results_temp = np.load(results_location+"/"+str(i)+".npy")
        # print(results_temp)
        all_combs = np.append(all_combs, results_temp)
    except:
        break
    i += 1
...
sim = load_json(save_location+"/kepler.json")

kepler_li = []
for order in range(len(all_combs)):
    sat = sim[str(order)]
            ...
            for i, x in enumerate(x_vals):
                for ii, y in enumerate(y_vals):
                    for iii in range(len(kepler_li)):
                        if kepler_li[iii,element1] == x and kepler_li[iii,element2] == y and conns[iii] !=0:
