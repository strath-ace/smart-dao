...
# Earthquake time
earthquake_timestamp = to_timestamp("2021-08-14 12:29:00")
...
x = []
min_val_li = []
words = ["earthquake", "haiti", "terremoto", "tsunami"]
twitter_words = load_json("./haiti_21-08-14/clean_twitter.json")
for word in words:
    temp = []
    split_x = []
    split_y = []
...
usgs_dates = load_json("./haiti_21-08-14/clean_usgs.json")
...
add_line("2021-08-14 22:20:00", "ICSMD Activation")


plt.stackplot(x, tweets_sum_li, all_stuff[0], all_stuff[1], alpha=0.6, labels=["100s of Tweets with Keywords", "USGS 'Did you feel it' Notifications", "Tsunami Warnings from GTS"])    
#plt.stackplot(x, tweets_sum_li, all_stuff[0], all_stuff[1], alpha=0.6, labels=["100s of Tweets with Keywords", "Media about event"])    
...
plt.title("Haiti Earthquake 14 Aug 2021")
plt.ylabel("Number of warnings")
plt.xlabel("Time since disaster (Hours)")
plt.savefig("haiti_2021-08-14")
