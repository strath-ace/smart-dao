...
const contract_name = "CalculateBeta";
...
async function main() {
  ...
  for (let h = min_dim; h <= max_dim; h++) {
    ...
    for (let j = 0; j < num_items; j++) {
      ...
      try{
        const result = await token.connect(await key_to_signer(accounts[0]))._compute_beta_seperate(matrix, tx_params);
  ...
  fs.writeFile("results.calculate_beta.txt", jsonData, function(err) {
    if (err) {
        console.log(err);
    }
  }); 
