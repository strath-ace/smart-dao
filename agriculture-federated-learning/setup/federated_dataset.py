...
class FederatedDataset:
    ...
    def _view_dataset(self) -> {}:
        if str(type(self._partitioners["train"])) == "<class 'flwr_datasets.partitioner.square_partitioner.SquarePartitioner'>":
            # Square Partitioner
            view_data = {
                "type_partition": "SQUARE",
                "num_partitions": self._partitioners["train"]._num_partitions,
                "full_set": [[i, i*i]  for i in range(1, self._partitioners["train"]._num_partitions+1)]
            }
        else:
            # IID Partitioner
            view_data = {
        ...
        print(view_data, self._partitioners["train"]._num_partitions)
        return view_data