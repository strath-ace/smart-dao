...
#     "../dataset-generator/dataset3/reference.json",
#      "../dataset-generator/dataset3",
#      "train"
# )
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.model, stored_data = train(self.model, trainloader, optimizer, epochs=epochs)
    ...
    def evaluate(self, parameters, config):
        ...
        # Construct dataloader
        valloader = DataLoader(self.valset, batch_size=2)

        # Evaluate
        loss, accuracy, _, _ = test(self.model, valloader)

        # Return statistics
        return float(loss), len(valloader.dataset), {"accuracy": float(accuracy)}


def get_client_fn(dataset):
...
def get_evaluate_fn(
    ...
    def evaluate(
        ...
        loss, accuracy, big_confusion, number_in_class = test(model, testloader)
        # loss = 0
        # accuracy = 0
        return loss, {"accuracy": accuracy, "number_in_class": number_in_class,"big_confusion": big_confusion.tolist()}
...
def main():
    ...
    centralized_testset = load_dataset("imagefolder", data_dir="../dataset-generator/dataset2/data")["test"]
    ...
    for j in range(1,params_config["TRAINING_ITERATIONS"]+1):
        ...
        if not os.path.exists(file_name):
            ...
            for i in TARGET_NUM_CLIENTS:
                ...
                print("##############")
                print("Run test with", NUM_CLIENTS, "clients")
                print("##############")

                dataset = load_dataset("imagefolder", data_dir="../dataset-generator/dataset2/data")
                
                partitioner = IidPartitioner(NUM_CLIENTS)
                partitioner.dataset = dataset["train"]
                                
                # partition_0 = partitioner.load_partition(partition_id=0)
