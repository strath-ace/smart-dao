...
def build(bin_sats):
    primary=0
    fit = BAD_FITNESS
    bin_sats = np.array(bin_sats)
    if np.sum(bin_sats) < 4:
        return BAD_FITNESS
    # print("Lower vals")
    sats_tle = np.array(g.load_json("sorted_sats.json"))
    # Reduce to only those in bin_sats
    sats_tle = sats_tle[:len(bin_sats)]
    sats_tle = sats_tle[bin_sats.astype(bool)]
